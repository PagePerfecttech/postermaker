...
app.post('/api/generate-poster', upload.fields([
    ...
    if (!template) {
      return res.status(404).json({ error: 'Template not found' });
        ...
        // Generate poster and save locally
        const outputFilename = `poster_${uuidv4()}.svg`;
        const outputPath = path.join(__dirname, 'uploads', 'generated', outputFilename);
        
        // Ensure generated directory exists
        if (!fs.existsSync(path.join(__dirname, 'uploads', 'generated'))) {
            ...
            if (field.type === 'text' && parsedTextFields && parsedTextFields[field.id]) {
                ...
                svgContent += `
                    ...
                    <text x="${x + 3}" y="${y + 3}" fill="rgba(0,0,0,0.9)" 
                          font-size="${fontSize}" font-family="${fontFamily}, sans-serif" 
        ...
        // Return local URL
        const posterUrl = `/uploads/generated/${outputFilename}`;
